pipeline {
  ...
  stages {
    ...
    stage ('Build stage') {
      steps {
        script {
          dockerImage = docker.build(registry, "-f ServerlessRealEstateApp-TeamAcacia/webapp/new-website/Dockerfile ServerlessRealEstateApp-TeamAcacia/webapp/new-website/Dockerfile")
        }
      }
    }
